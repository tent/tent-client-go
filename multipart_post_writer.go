...
func (w *MultipartPostWriter) WritePost(post *Post) error {
	...
	part, err := w.m.CreatePart(mimeFileHeader("post", "post.json", post.contentType(), len(data)))
	if err != nil {
		return err
	}
...
func mimeFileHeader(name, filename, contentType string, size int) textproto.MIMEHeader {
